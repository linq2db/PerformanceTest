...
namespace PerformanceTest.Views.MainWindow
{
	...
	partial class MainWindowViewModel
	{
		...
		public  AsyncRelayCommand  RefreshCommand => field ??= new AsyncRelayCommand(RefreshDataAsync);
			...
			try
			{
				...
				var ms =
					(
						...
						select new
						{
							...
							{
								Method  = mt,
								Watches = ws.TryGetValue(mt.ID, out var wt) ? wt : []
							}).ToList()
						}
					)
					.ToDictionary(t => t.Key, t => t.Items);
				...
				var rs =
					(
						...
						select new { g.Key, Items = g.Select(rt => new { Run = rt, Methods = ms.TryGetValue(rt.ID, out var m) ? m : []}).ToList() }
					)
					.ToDictionary(t => t.Key, t => t.Items);
		...
		public  AsyncRelayCommand  KillCommand => field ??= new AsyncRelayCommand(KillDataAsync);
